...
def plot_functions(_nb_epoch, _tr_loss, _val_loss, _f1, _er, extension=''):
    ...
    plot.savefig(opt.models_resnet34dir + __fig_name + extension)
    plot.close()
...
def train(model, train_data):
    model.train()
    train_dataloader = DataLoader(train_data, batch_size=128, shuffle=False, num_workers=opt.num_workers)
...
def eva(model, test_data):
    test_dataloader = DataLoader(test_data, batch_size=128, shuffle=False, num_workers=opt.num_workers)
...
print('TRAINING PARAMETERS: nb_ch: {}, seq_len: {}, batch_size: {}, nb_epoch: {}, frames_1_sec: {}'.format(
    opt.nb_ch, opt.seq_len, 128, opt.nb_epoch, opt.frames_1_sec))

utils.create_folder(opt.models_resnet34dir)
...
for fold in [2]:
    ...
    test_data = MelData(isTrain=False, feat_folder=opt.feat_folder, mono=opt.is_mono, fold=fold, seq_len=opt.seq_len,
                        nb_ch=opt.nb_ch)
    model.load_state_dict(torch.load('models_resnet34/bin_2022_05_09_15_54_14_fold_2_model', map_location='cpu'))
    ...
    for i in range(opt.nb_epoch):
        ...
        if er_overall_1sec_list[i] < best_er:
            ...
            model_path = os.path.join(opt.models_resnet34dir, '{}_fold_{}_model'.format(__fig_name, fold))
