...
def plot_functions(_nb_epoch, _tr_loss, _val_loss, _f1, _er, extension=''):
    ...
    plot.savefig(opt.models_cnndir + __fig_name + extension)
    plot.close()
...
utils.create_folder(opt.models_cnndir)
...
for fold in [2]:
    ...
    for i in range(opt.nb_epoch):
        ...
        if er_overall_1sec_list[i] < best_er:
            ...
            model_path = os.path.join(opt.models_cnndir, '{}_fold_{}_model'.format(__fig_name, fold))
            torch.save(model.state_dict(), model_path)
            # model.save(os.path.join(__models_cnndir, '{}_fold_{}_model.h5'.format(__fig_name, fold)))
            best_epoch = i
            pat_cnt = 0
