...
def plot_functions(_nb_epoch, _tr_loss, _val_loss, _f1, _er, extension=''):
    ...
    plot.savefig(opt.models_resnet50dir + __fig_name + extension)
    plot.close()
...
def train(model, train_data):
    model.train()
    train_dataloader = DataLoader(train_data, batch_size=64, shuffle=False, num_workers=opt.num_workers)
...
def eva(model, test_data):
    test_dataloader = DataLoader(test_data, batch_size=64, shuffle=False, num_workers=opt.num_workers)
...
print('TRAINING PARAMETERS: nb_ch: {}, seq_len: {}, batch_size: {}, nb_epoch: {}, frames_1_sec: {}'.format(
    opt.nb_ch, opt.seq_len, 64, opt.nb_epoch, opt.frames_1_sec))

utils.create_folder(opt.models_resnet50dir)
...
for fold in [2]:
    ...
    for i in range(opt.nb_epoch):
        ...
        if er_overall_1sec_list[i] < best_er:
            ...
            model_path = os.path.join(opt.models_resnet50dir, '{}_fold_{}_model'.format(__fig_name, fold))
