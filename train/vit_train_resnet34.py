...
class MelData(data.Dataset):
    ...
    def __load_data__(self, _feat_folder, _mono, _fold=None):
        if _fold != 0:
            feat_file_fold = os.path.join(_feat_folder, 'resnet50_data_fold{}.npz'.format(_fold))
        else:
...
for fold in [2] :
    ...
    if opt.use_gpu:
        model.cuda()
    model.load_state_dict(torch.load('weights/attcnn/2022_04_17_18_52_38_fold_{}_model'.format(fold)))
