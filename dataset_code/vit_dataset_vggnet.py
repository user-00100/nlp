...
def eva(model, test_data):
    ...
    test_pred = None
    tmp_data = torch.empty(0, 64).to(device)
...
for fold in [2]:
    ...
    Model().load_state_dict(torch.load('models_attvggnet/bin_2022_04_15_21_02_23_fold_{}_model'.format(fold)))
    ...
    normalized_feat_file = os.path.join(feat_folder, 'attvggnet_data_fold{}.npz'.format(fold))
